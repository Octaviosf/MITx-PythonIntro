...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for l in alphabetLower:
            try:
                shift_dict[l] = alphabetLower[self.shift]
            except IndexError:
                self.shift = 0
                shift_dict[l] = alphabetLower[self.shift]
            self.shift += 1
        ...
        for l in alphabetUpper:
            try:
                shift_dict[l] = alphabetUpper[self.shift]
            except IndexError:
                self.shift = 0
                shift_dict[l] = alphabetUpper[self.shift]
            self.shift += 1
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(27):
            ...
            decrypted_words = message_text_decrypted.split()

            for word in decrypted_words:
                valid_words.append(is_word(self.valid_words, word))

            num_valid_words = sum(valid_words)
            decrypt_dict[num_valid_words] = (26-shift, message_text_decrypted)

            valid_words = []

        max_valid_words = max(decrypt_dict.keys())

        return decrypt_dict[max_valid_words]
